...
DIC 
log GCPO = group conditional predictive ordinate

Prepare data set for making groups
```{r}
...
data_group$id<-data_group$mcp_id
data_group <- data_group %>% arrange(duplicateID, id)

groups = rep(list(list()),length(unique(data_group$id)))
for(i in 1:length(unique(data_group$id))){
    groups[[i]] = which(data_group$id == sort(unique(data_group$id))[i])
  }
...
formula.multmodel2 <-update.formula(baseformula, ~ . + basis_bc + basis_vaccine)
```

```{r}
formulas_mult <-list(formula.multmodel1,formula.multmodel2)
labels_mult <-c("model.bc_ozone","model.bc_vaccine")
```
...
```{r}
table1 <- data.table::data.table(Model  = c("model.bc_ozone", "model.bc_vaccine"),
                     DIC = NA, logCPO = NA)
...
```{r}
formula.multmodel4 <-update.formula(formula.multmodel1, ~ . + basis_vaccine)
```

```{r}
formulas_mult_3 <-list(formula.multmodel4)
labels_mult_3 <-c("model.bc_ozone_vaccine")
```

```{r}
models_mult_3 <- lapply(1:length(formulas_mult_3), 
              function(i) {
                model <- mymodel(formulas_mult_3[[i]], df)
                save(model, file = paste0("output/models/", labels_mult_3[i],".RData"))})
```


```{r}
table3 <- data.table::data.table(Model  = c( "model.bc_ozone_vaccine"),
                                 DIC = NA, logCPO = NA)

for(i in 1:length(labels_mult_3))
  {
  load(paste0("output/models/",labels_mult_3[i],".RData"))
  table3$DIC[i] <- round(model$dic$dic, 0)
  table3$logCPO[i] <- round(-mean(log(model$cpo$cpo), na.rm=T), 4)
  #gcpo = inla.group.cv(model,  groups = groups, strategy = "posterior")
  #table3$logGCPO[i] <- round(-mean(log(gcpo$cv), na.rm=T), 4)
}
# view table
table3
```

Summary: comparing the models only with BC with models containing other covariate except BC, I see that including ozone and vaccine in addition to BC, improves the model. 




Change time random effect prior 
```{r}
precision.prior_smooth1 <- list(prec = list(prior = "pc.prec", param = c(0.05, 0.001)))
precision.prior_smooth2 <- list(prec = list(prior = "pc.prec", param = c(0.005, 0.0001)))
precision.prior_smooth3 <- list(prec = list(prior = "pc.prec", param = c(0.0005, 0.00001)))
precision.prior_smooth4 <- list(prec = list(prior = "pc.prec", param = c(0.00005, 0.000001)))

formula_smooth1 <- Y ~ f(T1,  model = "rw2", constr = TRUE, scale.model = TRUE, hyper = precision.prior_smooth1 ) +
  f(S1, model = "bym2",graph = "output/adjacency.mat", scale.model = TRUE, hyper = precision.prior)

formula_smooth2 <- Y ~ f(T1,  model = "rw2", constr = TRUE, scale.model = TRUE, hyper = precision.prior_smooth2 ) +
  f(S1, model = "bym2",graph = "output/adjacency.mat", scale.model = TRUE, hyper = precision.prior)

formula_smooth3 <- Y ~ f(T1,  model = "rw2", constr = TRUE, scale.model = TRUE, hyper = precision.prior_smooth3 ) +
  f(S1, model = "bym2",graph = "output/adjacency.mat", scale.model = TRUE, hyper = precision.prior)

formula_smooth4 <- Y ~ f(T1,  model = "rw2", constr = TRUE, scale.model = TRUE, hyper = precision.prior_smooth4 ) +
  f(S1, model = "bym2",graph = "output/adjacency.mat", scale.model = TRUE, hyper = precision.prior)

formula.model_smooth1 <- update.formula(formula_smooth1, ~ . +  basis_bc + basis_ozone + basis_vaccine)
formula.model_smooth2 <- update.formula(formula_smooth2, ~ . +  basis_bc + basis_ozone + basis_vaccine)
formula.model_smooth3 <- update.formula(formula_smooth3, ~ . +  basis_bc + basis_ozone + basis_vaccine)
formula.model_smooth4 <- update.formula(formula_smooth4, ~ . +  basis_bc + basis_ozone + basis_vaccine)

formulas_smooth <-list(formula.model_smooth1,formula.model_smooth2,formula.model_smooth3, formula.model_smooth4)
labels_smooth <-c("model_smooth1", "model_smooth2", "model_smooth3", "model_smooth4")

models_smooth<- lapply(1:length(formulas_smooth), 
              function(i) {
                model <- mymodel(formulas_smooth[[i]], df)
                save(model, file = paste0("output/models/", labels_smooth[i],".RData"))})


```
```{r}
table4 <- data.table::data.table(Model  = c("model.bc_ozone_vaccine", "smooth1",
                                            "smooth2", "smooth3", "smooth4"),
                                 DIC = NA, logCPO = NA)

  load(paste0("output/models/model.bc_ozone_vaccine.RData"))
  table4$DIC[1] <- round(model$dic$dic, 0)
  table4$logCPO[1] <- round(-mean(log(model$cpo$cpo), na.rm=T), 4)
  #gcpo = inla.group.cv(model,  groups = groups, strategy = "posterior")
  #table4$logGCPO[1] <- round(-mean(log(gcpo$cv), na.rm=T), 4)
  
for(i in 2:(length(labels_smooth)+1))
  {
  load(paste0("output/models/",labels_smooth[i-1],".RData"))
  table4$DIC[i] <- round(model$dic$dic, 0)
  table4$logCPO[i] <- round(-mean(log(model$cpo$cpo), na.rm=T), 4)
  #gcpo = inla.group.cv(model,  groups = groups, strategy = "posterior")
  #table4$logGCPO[i] <- round(-mean(log(gcpo$cv), na.rm=T), 4)
}
# view table
table4
```

DIC and logCPO point towards our original model as the best one.
...
Based on the comparison between 1-covariate spatial-temporal model, we still can choose a model with BC.

# Spatial-temporal models and multiple covariates models

I have tried the models including, bc, ozone and, in addition, pm25. But they crush, rerun 3 times with the same result. 
...
Spatial-temporal models suggest an effect of BC, but when adding other pollutants the model does not really improve (no further decrease in DIC).
