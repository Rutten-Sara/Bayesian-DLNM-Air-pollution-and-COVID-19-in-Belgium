...
# Population density
```{r}
map_key <- st_read("../data/belgium_shape/Apn_AdMu.shp") %>%
  rename("mcp_code"="AdMuKey") %>%
  mutate(mcp_code = as.numeric(mcp_code))

data_sort <- left_join(data_sort, map_key, by = "mcp_code") %>%
  mutate(pop_dens = pop_tot/Shape_Area*1000)

dep <- read.csv("../data/deprivation.csv") %>%
  rename("mcp_code"="municipality") %>%
  mutate(mcp_code = as.numeric(mcp_code))%>%
  mutate(avg_score = avg_score/100)

data_sort <- left_join(data_sort, dep, by = "mcp_code") 
...
pop_dens <- data_prep$pop_dens
avg_score <- data_prep$avg_score
```

Create a dataframe for model testing

```{r}
df <- data.frame(Y, E, T1,S1, T1.1, S1.T1, pop_dens, avg_score)
```
